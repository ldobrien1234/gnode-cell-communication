...
for epoch in range(train_epochs):
    ...
    for inputs, targets in train_loader:
        ...
        inputs = inputs.to(device) #dimension (batch_size, no. features) 
        targets = targets.to(device) #dimension (batch_size, num_evals, no. features)
        
        #output matrix from odeint: the features at every evaluation time
        #tensor dimension is (num_evals, batch_size, no. features)
...
torch.save(model.state_dict(), "Cell1_Model.pt")
...
text_file = open("cell1_loss.txt", "w")
